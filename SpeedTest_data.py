...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 22 20:27:49 2020
...
class DBObj(object):
    ...
    def read_tick1(self, table_name, start_date=None, end_date=None, symbol=None):
        ...
        for x in collection.find(query): 
            ...
            segs.append(x) 
        segs.sort(key=lambda x: (x['symbol'], x['date'], x['start'])) 
        return pd.DataFrame(np.concatenate([x['data'] for x in segs], axis=0), columns=x['data'].columns).astype(x['data'].dtypes.to_dict())
...
timelist = db1.read_tick('md_index', 20190102, 20191231, symbol=1000300)['date'].unique()
t0 = timelist[0]
data1 = []
for t in timelist:
    data = db1.read_tick('md_snapshot_l2', start_date=str(t), end_date=str(t), symbol=2000006)
    data.to_pickle('/data/home/zhenyu/DBdata/per_day/' + str(t) +'.pkl')
    if str(t)[4:6] == str(t0)[4:6]:
        data1 += [data]
    else:
        data1 = pd.concat(data1).reset_index(drop=True)
        data1.to_pickle('/data/home/zhenyu/DBdata/per_month/' + str(t0)[4:6] +'.pkl')
        data1 = []
        data1 += [data]
    t0 = t
data1 = pd.concat(data1).reset_index(drop=True)
